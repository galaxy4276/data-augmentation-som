...
export function useTaskStatus(taskId: string | null | undefined) {
  ...
  const { notifyPending, notifyRunning, notifyCompleted, notifyFailed } = useTaskNotification();
    ...
    if (!notifiedStatuses.has(notificationKey)) {
      switch (status) {
        ...
        case 'completed':
          ...
          break;
        case 'failed':
          notifyFailed(taskType, query.data.error || '알 수 없는 오류', (error) => {
  ...
  }, [query.data, taskId, notifiedStatuses, notifyPending, notifyRunning, notifyCompleted, notifyFailed, getTaskType]);
