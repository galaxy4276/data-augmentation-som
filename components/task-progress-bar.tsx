...
export function TaskProgressBar({ taskId, onComplete }: TaskProgressBarProps) {
  ...
  const { status, progress, current_step, total_steps, estimated_completion, error } = taskStatus;
    ...
    <>
      <div className="p-4 border rounded-lg bg-card">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${statusTextColors[status]} bg-opacity-10 ${statusColors[status].replace('bg-', 'bg-opacity-10 text-')}`}>
              {statusLabels[status]}
            </span>
          </div>
      ...
      <div className="space-y-1 text-xs text-muted-foreground">
        ...
        {total_steps > 0 && (
          <div className="flex items-center justify-between">
            ...
            <span>Step {Math.ceil((progressPercent / 100) * total_steps)} / {total_steps}</span>
          </div>
        )}
      ...
      {status === 'pending' && (
        <div className="mt-3 p-2 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded text-xs text-yellow-700 dark:text-yellow-300">
          <span className="font-medium">대기 중:</span> 태스크가 큐에 대기 중입니다. 잠시 후 시작됩니다.
        </div>
      )}
      ...
      {status === 'running' && (
        <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded text-xs text-blue-700 dark:text-blue-300">
          <span className="font-medium">진행 중:</span> 데이터를 추출하고 처리하고 있습니다.
        </div>
      )}
      ...
      {status === 'completed' && (
        <div className="mt-3 p-2 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded text-xs text-green-700 dark:text-green-300">
          <span className="font-medium">완료!</span> 데이터 추출이 성공적으로 완료되었습니다.
        </div>
      )}
